...
인터랙티브 데모 **10개** · 실험 **21개** · 사전 등록 주장 **72개**(34 성립 · 27 반증 · 11 부분·보류) · **실측 사이트 6곳**
...
> **"도움이 된다. 단, 다운로드를 빠르게 해서가 아니라 `loading="lazy"` 이미지의
> 요청 발사 시점을 앞당겨서다. LCP 이미지가 `eager` 라면 그 경로가 없다."**

<sub>사전 등록 [PREDICTION.md](experiments/track-c/c8-image-lcp/PREDICTION.md) · 전체 결과 [RESULTS.md](experiments/track-c/c8-image-lcp/RESULTS.md) — 주장 10개 중 **2개 반증 · 3개 부분**</sub>
...
| LCP 이미지가 `eager` 인 실제 사이트에서도 같은가 | ❌ 미검증. `eager` 는 내가 HTML 을 고쳐 만든 조건이다 |
