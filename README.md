...
인터랙티브 데모 **10개** · 실험 **16개** · 사전 등록 주장 **53개**(25 성립 · 23 반증 · 5 부분·보류)

**내가 틀린 것 15개** · 폐기한 측정법 **6개**
...
- [**Track C · 앞의 결론을 되짚는다**](#track-c--track-a-의-결론을-되짚는다)
  ...
  - [C5 · "이득은 Style" 은 최악 조건에서만 참이었다](#c5--이득은-style-은-최악-조건에서만-참이었다)
- [통용되는 말 vs 실측](#통용되는-말-vs-실측)
- [내가 틀렸던 것 15번](#내가-틀렸던-것-15번)
- [방법론 19조](#방법론-19조)
- [재현하기](#재현하기)
- [파일 구조](#파일-구조)
- [참고 자료](#참고-자료)
...
> ⚠️ **이 문장은 [C5](#c5--이득은-style-은-최악-조건에서만-참이었다) 에서 범위가 좁혀진다.**
> 위 세 무효화는 **전부 `:root` 커스텀 프로퍼티**라 트리 전체를 무효화하는 최악 경우였다.
> 현실적인 무효화로 다시 재면 **이득이 Layout 에서 나오거나, 아예 손해**다.
...
#### 그 26배마저 Chromium 이 아니라 내 측정 절차였다

평균뿐 아니라 **중앙값도** 같이 움직였다(p50 39 → 963). 분포 모양은 그대로인데 통째로 오르내린다 —
실행 **단위**의 문제라는 뜻이다. 그래서 같은 설정만 10회씩, **격리 조건만** 바꿔 재봤다.

| 조건 | 잔여 chrome 프로세스 | **p50 흩어짐** |
|---|---|---:|
| 현행 (`kill` 후 0.5초) | 11~52 | **6.1배** |
| **엄격 격리** (이전 브라우저가 완전히 끝날 때까지 + 3초) | 11~13 | **1.6배** |
| 엄격 격리 + 빌드 전 5초 워밍업 | 11~22 | 1.3배 |

**이전 실행이 끝나기를 기다리는 것만으로 흩어짐이 무너진다.** 워밍업은 얻는 게 없었다.

다만 **프로세스 "개수" 가 원인은 아니다** — 개수와 p50 의 상관이 **r = −0.382** 로 약하고 음수다.
정리가 덜 끝난 상태(프로세스 종료·GPU 컨텍스트 반납·프로필 삭제 I/O)가 측정을 **양쪽으로** 흔드는 것이고,
개수는 그 상태의 표시등일 뿐이다.
...
| | |
|---|---|
| 세션 **안** · 엄격 격리 | 2배 이상이면 잴 수 있다 |
| 세션 **안** · 현행 | 6배 이상이어야 잰다 |
| 세션 **사이** | **못 잰다** — 같은 설정이 8~51µs(이번) vs 39~963µs(직전) |

> 앞에서 *"이 하네스로는 타일당 비용을 비교할 수 없다"* 고 썼는데 **절반만 맞았다.**
> 엄격 격리를 쓰고 한 세션 안에서 비교하면 2배까지는 잴 수 있다. 내가 그렇게 안 했을 뿐이다.

**이건 이 저장소 전체에 해당한다.** [A2 에서 "설정마다 브라우저 새로 기동"](#a2--합성-레이어의-gpu-메모리)을
도입한 건 맞는 판단이었지만, **종료 대기를 0.5초로 둔 것**이 그때부터 모든 시간 측정에 섞여 있었다.
횟수·개수로 낸 결론은 영향 없고, 시간으로 낸 것 중 **차이가 2~6배 미만**인 건 다시 봐야 한다.
Track A 의 주요 결론들은 배율이 그보다 커서(Layout 32배 · Paint 94배 · Style 82%) 살아남는다.
>
...
### C5 · "이득은 Style" 은 최악 조건에서만 참이었다

> **질문** — C2 의 결론이 이 저장소에서 제일 쓸모 있을 수 있는데, **무효화 조건이 하나뿐**이었다. 일반화되나?

C2 의 세 무효화는 전부 **`:root` 의 CSS 커스텀 프로퍼티**였다. 커스텀 프로퍼티는 **상속**되므로
트리 전체의 스타일이 무효화된다 — **스타일 재계산의 최악 경우**다.
커스텀 프로퍼티를 쓰지 않는 무효화 셋을 더해 다시 쟀다.

#### 결과 — 정반대다

| 무효화 | 이득의 출처 | 전체 |
|---|---|---|
| **컨테이너 폭만** (가장 흔함) | **Layout** (165.8 → 57.3ms). Style 은 **오히려 늘어난다**(0.41 → 9.29) | **2.73배 이득** |
| 항목 스타일 + 레이아웃 | Layout · PrePaint. Style 은 +40% | 1.53배 이득 |
| **항목 스타일만** | 없음 | **4.5배 손해** |
| `:root` 변수 (C2 가 쓴 방식) | **Style** (138.2 → 57.2ms) | 1.32배 이득 |

**C2 가 본 "Style 절감" 은 네 무효화 중 하나에서만 나타난다.** 하필 C2 가 쓴 그 하나다.

#### 왜 그랬나 — `plain` 의 Style 이 337배 차이난다

| 무효화 | `plain` 의 Style (N=4,000) |
|---|---:|
| 컨테이너 폭만 | **0.41 ms** |
| 항목 스타일만 | 22.95 ms |
...
**건너뛸 스타일 작업이 애초에 없으면 `content-visibility` 가 절감할 것도 없다.**
C2 는 가장 비싼 칸 하나만 보고 "Style 이 지배한다" 고 결론냈다.

#### 제일 나쁜 칸 — 항목 스타일만 바뀔 때 (색 테마 토글 같은 것)
...
| **실제** | "**무효화가 트리 전체의 스타일을 건드릴 때만** Style 에서 이득이 난다. 컨테이너만 바뀌는 흔한 경우엔 이득이 **Layout** 에서 나오고, 항목 스타일만 바뀌면 **손해**다" |

C2 의 결론은 틀린 게 아니라 **조건이 빠져 있었다.** 그런데 그 조건이 특수해서 문장만 읽으면 **반대로 안내한다.**

<sub>사전 등록 [PREDICTION.md](experiments/track-c/c5-realistic-invalidation/PREDICTION.md) · 전체 결과 [RESULTS.md](experiments/track-c/c5-realistic-invalidation/RESULTS.md) — 주장 5개 중 **2개 반증**
· [C4d 의 엄격 격리](#c4--raster-110배는-내가-정한-2초였다)를 처음 적용한 실험 (조건 순서도 섞음)</sub>
...
| "`content-visibility` 는 layout 과 paint 를 건너뛴다" | ⚠️ **무효화 종류가 전부를 정한다.** 컨테이너 폭 변경이면 **Layout 에서** 2.9배 이득, 항목 스타일만 바뀌면 **4.5배 손해** |
| "`content-visibility` 의 이득은 Style 절감이다" | ❌ **`:root` 커스텀 프로퍼티라는 최악 조건에서만.** 흔한 경우엔 Style 이 **오히려 늘어난다**(0.41 → 9.29ms) |
...
| 13 | "26배는 GPU 큐 대기다 (도구의 한계)" | C4d 격리 조건 | **내 절차였다.** 이전 브라우저를 기다리면 **1.6배**로 떨어진다 |
| 14 | "content-visibility 의 이득은 Style 에서 나온다" | C5 현실적 무효화 | **최악 조건에서만.** 흔한 경우엔 이득이 **Layout** 에서 나온다 — 정반대 |
| 15 | "항목 스타일을 건드리면 cv 가 Style 을 아낄 것" | C5 (사전 등록 P2) | **오히려 늘어난다**(+40% · +209%). 내 모형이 단순했다 |
...
| 19 | [C4d](#c4--raster-110배는-내가-정한-2초였다) — `kill` 후 0.5초면 정리가 안 끝난다. 기다리기만 해도 흩어짐이 **6.1배 → 1.6배** |
...
│   ├── c4-raster-threshold/     PREDICTION ×4 · measure · probe-settle · probe-why
│                                probe-occlusion · probe-tilecost · probe-overlap
│   │                            probe-gpuvar · RESULTS
│   └── c5-realistic-invalidation/  PREDICTION · index.html · measure · analyze · RESULTS
│
└── report/
...
| ~~GPU 래스터에서 `dur` 이 왜 26배 널뛰나~~ | ✅ **닫음** — Chromium 이 아니라 **내 측정 절차**(이전 브라우저 종료 대기 0.5초) |
| **세션 사이의 10배 드리프트** | ❌ 미확인. 엄격 격리는 세션 **안**만 고친다 |
| 정리 미완 상태가 왜 측정을 **양쪽으로** 흔드나 | ❌ 미확인. 프로세스 개수와는 상관이 약하다(r=−0.38) |
