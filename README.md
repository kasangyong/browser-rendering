...
인터랙티브 데모 **10개** · 실험 **17개** · 사전 등록 주장 **58개**(27 성립 · 24 반증 · 7 부분·보류)

**내가 틀린 것 17개** · 폐기한 측정법 **6개**
...
- [**Track C · 앞의 결론을 되짚는다**](#track-c--track-a-의-결론을-되짚는다)
  ...
  - [C6 · 실제 웹사이트에서 — 합성 하네스가 양쪽으로 틀렸다](#c6--실제-웹사이트에서--합성-하네스가-양쪽으로-틀렸다)
- [통용되는 말 vs 실측](#통용되는-말-vs-실측)
- [내가 틀렸던 것 17번](#내가-틀렸던-것-17번)
- [방법론 19조](#방법론-19조)
- [재현하기](#재현하기)
- [파일 구조](#파일-구조)
- [참고 자료](#참고-자료)
...
| 무효화 | 합성 C5 | **실제 C6** | |
|---|---:|---:|---|
| 컨테이너 폭만 | 2.73배 이득 | **11.30배 이득** | 4배 더 유리 |
| 항목 스타일만 | **4.5배 손해** | **1.01배 (무승부)** | ⚠ **다름** |
| `:root` 변수 | 1.32배 이득 | **5.59배 이득** | 4배 더 유리 |

합성에서 `plain` 의 폭 변경 Style 은 **0.41ms**, 실제는 **77ms — 188배**다.
복잡한 선택자·수천 개 규칙·47,925 노드. **건너뛸 일이 많으니 절감할 것도 많다.**
C5 에서 *"건너뛸 스타일 작업이 없으면 절감할 것도 없다"* 고 썼는데,
**내 하네스가 바로 그 "작업이 없는" 쪽**이었다.
...
|---|---:|
| 원본 | **152,571 px** |
| `cv-auto` (`auto 81px`) | **16,303 px** |
| 오차 | **−89.3%** |

| | 블록 수 | 높이 | **변동계수** |
|---|---:|---|---:|
| 내 합성 리스트 | 4,000 | 109~109px | **≈ 0** |
| **실제 Wikipedia** | 698 | **26 ~ 43,220px** | **3.72** |

26px 짜리와 43,220px 짜리에 **같은 추정치**를 줘야 한다. `auto` 가 렌더된 크기를 기억하지만
대부분의 블록은 화면 밖이라 **한 번도 렌더된 적이 없다.**
...
> 진짜 문제는 **문서 높이**다 — 블록 크기가 제각각이면 `contain-intrinsic-size` 를 한 값으로 줄 수 없고
> 스크롤바가 깨진다. **적용 전에 블록 높이의 분포부터 보라.**
...
| "`contain-intrinsic-size` 는 값만 잘 주면 된다" | ⚠️ **실제 페이지에선 줄 수 있는 값이 없다.** 블록 높이가 26~43,220px(변동계수 3.72) — 한 값으로는 문서가 **89% 틀린다** |
| "`content-visibility` 는 아무 페이지에나 붙일 수 있다" | ❌ 조사한 문서 사이트 **4곳 중 1곳**만 가능. 반복 단위가 `<tr>` 이면 **`contain: size` 가 적용 안 된다** |
...
│   └── c6-real-sites/           PREDICTION · probe-structure/target/granularity
│                                measure · analyze · RESULTS  (실제 Wikipedia)
│
└── report/
