...
인터랙티브 데모 **10개** · 실험 **13개** · 사전 등록 주장 **40개**(21 성립 · 15 반증 · 4 부분·보류)

**내가 틀린 것 11개** · 폐기한 측정법 **5개**
...
> 앞서 *"가림 가설 기각"* 이라고 썼는데 **그 기각이 틀렸다.** 그때는 래스터 **횟수**만 봤고
> (1,374 vs 1,205, 거의 같음), 겹침이 횟수가 아니라 **개당 비용**을 바꾼다는 걸 놓쳤다.

**그리고 이건 내 하네스의 한계를 드러낸다.** C3·C4 의 모든 측정은 박스를 전부 `left:0` 에 쌓아 왔고,
그게 타일 비용을 5~10배 부풀리는 배치였다. 겹침 조건이 모든 설정에서 같았으므로
**상대 비교는 유효하지만 절대값은 대표적이지 않다.**
...
| 11 | "가림은 원인이 아니다" (횟수가 같으니까) | C4b 겹침 변수 | **기각이 틀렸다.** 겹침은 횟수가 아니라 **개당 비용**을 5~10배 바꾼다 |
...
│   └── c4-raster-threshold/     PREDICTION ×2 · measure · probe-settle · probe-why
│                                probe-occlusion · probe-tilecost · RESULTS
│
└── report/
...
| **겹침이 왜 타일당 비용을 10배로 만드나** | ❌ 미확인. 타일 내용은 같은데 `dur` 이 10배다. 대기·경합이 섞였을 수 있지만 미검증 |
