...
인터랙티브 데모 **10개** · 실험 **33개** · 사전 등록 주장 **102개**(51 성립 · 36 반증 · 15 부분·보류) · **실측 사이트 18곳**

**내가 틀린 것 19개** · 폐기한 측정법 **6개**
...
- [**Track C · 앞의 결론을 되짚는다**](#track-c--track-a-의-결론을-되짚는다)
  ...
  - [C6 · 실제 웹사이트 3곳에서 — 합성 하네스가 양쪽으로 틀렸다](#c6--실제-웹사이트-3곳에서--합성-하네스가-양쪽으로-틀렸다)
  - [C7 · 초기 로드의 실제판 — "LCP 36~39% 단축" 은 조건부였다](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다)
  - [C8 · LCP 가 이미지일 때 — 빨라지긴 하는데, 이유가 달랐다](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다)
- [통용되는 말 vs 실측](#통용되는-말-vs-실측)
- [내가 틀렸던 것 25번](#내가-틀렸던-것-25번)
- [방법론 23조](#방법론-23조)
- [재현하기](#재현하기)
- [파일 구조](#파일-구조)
- [참고 자료](#참고-자료)
...
> ⚠️ **이 숫자는 [C7](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다) 에서 조건이 붙는다.**
> 실제 사이트 3곳 중 **둘에서는 LCP 가 안 움직인다.** 이 합성 리스트가
> **레이아웃이 병목인 페이지**였기 때문에 나온 값이다.
>
> 그리고 [C8](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다) 에서 **기전도 달라진다.**
> 이 합성 리스트에는 이미지가 없다. LCP 가 이미지인 실제 페이지에서 LCP 가 빨라지는 건
> 레이아웃이 짧아져서가 아니라 **`loading="lazy"` 이미지의 요청이 앞당겨져서**다.
...
### C6 · 실제 웹사이트 3곳에서 — 합성 하네스가 **양쪽으로** 틀렸다

> **질문** — 16개 실험이 전부 **내가 만든 합성 리스트**였다. 실제 페이지에서도 같은 결론이 나오나?

대상: **실제 사이트 3곳** — 생성기가 서로 다르다 (MediaWiki · 자체 빌드 스펙 · Sphinx)
(공개 페이지를 읽기만 했다. 총 65회 로드)

| 사이트 | 블록 | DOM | 문서 높이 | **변동계수** |
|---|---:|---:|---:|---:|
| Wikipedia (Unicode 목록) | 698 | 47,925 | 152,571px | **3.72** |
| WHATWG HTML Standard | 410 | 9,320 | 64,120px | **1.90** |
| Python 문서 | 77 | 7,011 | 37,059px | **1.16** |

#### 결론 — 방향이 반대로 갈린다

| | 성능 | 정확성 |
|---|---|---|
| **합성이 말한 것** | cv 는 조건에 따라 **4.5배 손해** | `contain-intrinsic-size` 하나로 **오차 1.3%** |
| **실제** | **11.3배 이득**, 최악도 무승부 | **오차 89.3%** — 스크롤바가 깨진다 |

**성능은 내가 과소평가했고, 정확성 문제도 내가 과소평가했다.**

#### 성능 — 실제가 훨씬 유리하다

| 무효화 | 합성 C5 | Wikipedia | WHATWG | Python | 일관성 |
|---|---:|---:|---:|---:|---|
| **컨테이너 폭만** | 2.73배 | **11.30배** | **5.89배** | **6.14배** | **3곳 모두 큰 이득** |
| **항목 스타일만** | **0.22배 (4.5배 손해)** | 1.01배 | 0.75배 | 1.28배 | **3곳 모두 무승부** |
| `:root` 변수 | 1.32배 | 5.59배 | 6.26배 | 1.42배 | 2곳 이득 · 1곳 무승부 |

**C5 가 "항목 스타일만 바뀌면 4.5배 손해" 라고 경고한 건 세 사이트 어디에서도 일어나지 않는다.**

#### 그런데 이득의 출처는 Style 이 아니다

Style 만 떼어 보면 (`plain` → `cv`, 컨테이너 폭 변경):

| Wikipedia | WHATWG | Python |
|---:|---:|---:|
| 77.02 → 8.53 (**−89%**) | 0.23 → 0.32 (**+40%**) | 0.48 → 0.68 (**+41%**) |

**세 곳 모두 전체로는 큰 이득인데 Style 은 두 곳에서 오히려 늘어난다** → **이득은 Layout 에서 온다.**

Wikipedia 만 Style 이 줄어드는 건 `plain` 의 Style 이 **77ms** 로 유독 비싸기 때문이다
(WHATWG 0.23 · Python 0.48). DOM 이 5배 크고 선택자가 복잡하다.
C5 에서 *"건너뛸 스타일 작업이 없으면 절감할 것도 없다"* 고 썼는데,
**내 하네스와 나머지 두 사이트가 바로 그 "작업이 없는" 쪽**이었다.

#### 정확성 — 여기서 실제가 훨씬 나쁘고, 오차가 변동계수를 따라간다

`contain-intrinsic-size` 를 관측된 중앙값으로 주고 문서 높이를 봤다.

| 사이트 | 변동계수 | 원본 | cv 적용 후 | **오차** |
|---|---:|---:|---:|---:|
| Wikipedia | 3.72 | 152,571px | 16,303px | **−89.3%** |
| WHATWG | 1.90 | 64,120px | 33,756px | **−47.4%** |
| Python 문서 | 1.16 | 37,059px | 25,149px | **−32.1%** |
| *(내 합성 리스트)* | *≈ 0* | *436,074px* | *—* | *−1.3%* |

**세 점이 단조롭다.** 블록 높이가 균일할수록 맞추기 쉽다.
그런데 **가장 균일한 Python 문서도 32% 틀린다** — 문서가 1/3 짧아진다.
어느 사이트에서도 스크롤바를 그대로 쓸 수 없다.

Wikipedia 블록은 **26px ~ 43,220px**. 그 둘에 **같은 추정치**를 줘야 한다.
`auto` 가 렌더된 크기를 기억하지만 대부분의 블록은 화면 밖이라 **한 번도 렌더된 적이 없다.**

#### 걸 곳을 찾는 것부터 문제였다

| 걸림돌 | |
|---|---|
| 반복 단위가 대부분 `<tr>` | **`contain: size` 는 테이블 내부 요소에 적용되지 않는다** |
| 권장대로 본문 직계 자식에 걸면 | Wikipedia 19개 · MDN 3개 · WHATWG 3개뿐 |

**조사한 4곳 중 1곳만 쓸 수 있었다.**

#### 그래서 실무자에게 할 말

> `content-visibility` 는 실제 페이지에서 **성능상 거의 항상 이득**이다 —
> 컨테이너 폭이 바뀌는 흔한 경우 **5.9~11.3배**, 최악도 무승부. 손해 보는 경우를 못 찾았다.
> 그리고 **그 이득은 Layout 에서 온다**(Style 은 오히려 늘 수도 있다).
>
> 진짜 문제는 **문서 높이**다. 블록 높이의 **변동계수**가 곧 오차다 —
> 1.16 이어도 32%, 3.72 면 89% 틀린다. **적용 전에 블록 높이의 분포부터 보라.**

<sub>사전 등록 [PREDICTION.md](experiments/track-c/c6-real-sites/PREDICTION.md) · 전체 결과 [RESULTS.md](experiments/track-c/c6-real-sites/RESULTS.md) — 주장 5개 중 **1개 반증 · 2개 부분**. 틀린 방향이 전부 "실제가 cv 에 유리" 쪽으로 일관됐다</sub>

---

### C7 · 초기 로드의 실제판 — "LCP 36~39% 단축" 은 조건부였다

> **질문** — 이 저장소에서 제일 인용하기 좋은 숫자가 **제일 검증이 안 돼 있었다.**
> [C1 의 "LCP 36~39% 단축"](#c1--a4-의-결론은-절반만-맞았다)은 합성 리스트에서 나왔고,
> [C6 가 그 하네스는 양쪽으로 틀렸다는 걸 보여줬다](#c6--실제-웹사이트-3곳에서--합성-하네스가-양쪽으로-틀렸다).

`content-visibility` 가 팔리는 문구는 **"초기 렌더가 빨라진다"** 한 줄이다. 실제 3곳에서 쟀다.

#### 결과 — 렌더링 일은 어디서나 줄지만, 사용자가 느끼는 건 한 곳뿐
...
판정은 사전 등록한 대로 **2배 미만이면 "차이 없음"**.

**Wikipedia 의 `plain` 은 첫 페인트 전에 레이아웃에만 22초를 쓴다.** cv 를 걸면 203ms —
24초가 2.2초가 된다. 반면 보통 크기 페이지에서는 **아무 일도 일어나지 않는다.**

#### 왜 갈리나 — 레이아웃이 병목이어야 보인다

반복별 LCP 를 펼치면 분명하다.

| 사이트 | 모드 | 반복별 LCP (ms) | 흩어짐 |
|---|---|---|---:|
| WHATWG | `plain` | 402 · 570 · 600 · 728 · 900 · 1752 | 4.4배 |
...
작은 페이지는 두 분포가 **완전히 겹친다** — 네트워크가 렌더링 차이를 덮는다.
Wikipedia 는 `cv-auto` 전체가 `plain` 아래에 있고, **흩어짐도 12.6배 → 2.2배로 줄었다.**

#### 합성과 대조하면 — 숫자가 아니라 **조건**이 빠져 있었다

| | 합성 C1 | 실제 C7 |
|---|---|---|
| 초기 Layout 절감 | 82% | 84% · 90% · **99%** |
| LCP 변화 | **−36~39%** | **차이 없음 · 차이 없음 · −91%** |

합성 리스트(4,000항목 · 436,074px)는 **레이아웃이 병목인 페이지**였다.
그런 페이지에서는 C1 이 맞는다 — Wikipedia 가 바로 그 경우다.
**보통 크기 페이지에서는 레이아웃이 병목이 아니다.**

> **그래서 문구는 이렇게 바뀐다:**
> *"`content-visibility` 는 초기 렌더를 빠르게 한다"* →
> **"레이아웃이 첫 페인트의 병목일 때만 빠르게 한다. 렌더링 일은 어디서나 84~99% 줄지만,
> 그게 사용자에게 보이려면 페이지가 충분히 커야 한다."**
> 기준이 필요하면: Layout 이 **수백 ms 대**면 안 보이고, **수 초 대**면 극적이다.

<sub>사전 등록 [PREDICTION.md](experiments/track-c/c7-real-initial-load/PREDICTION.md) · 전체 결과 [RESULTS.md](experiments/track-c/c7-real-initial-load/RESULTS.md) — 주장 4개 중 **1개 반증 · 1개 측정 불가**</sub>

---

### C8 · LCP 가 이미지일 때 — 빨라지긴 하는데, 이유가 달랐다

> **질문** — [C7 의 3곳은 전부 FCP = LCP 였다.](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다)
> 첫 페인트에 나온 텍스트가 그대로 LCP 요소라, 사전 등록한 주장 하나를 **측정조차 못 했다.**
> 실제 웹에서 LCP 는 대개 **이미지**고, 이미지 LCP 는 레이아웃이 아니라 **다운로드**에 묶여 있다.

**예측: 안 빨라진다.** 틀렸다.

#### 대상을 찾는 데만 세 번 걸렸다

데스크톱 1280px 에서는 긴 위키 문서 8곳이 **전부 텍스트 LCP** 였다 — 썸네일이 작아 문단 면적이 더 크다.
갤러리 페이지는 이미지 LCP 지만 문서가 1,519~3,076px 라 `content-visibility` 가 의미 없다.

**모바일 폭(390×844)에서 뒤집혔다.** 문단 면적이 줄고 이미지가 전체 폭을 차지한다.
실제 트래픽 대부분이 그 폭이기도 하다.

| 사이트 | 문서 높이 | 이미지 | LCP 요소 | 블록 |
|---|---:|---:|---|---:|
| Grand Canyon | 48,963px | 69 | `<img>` 53,280px² | 224 |
| Milky Way | 71,931px | 46 | `<img>` 52,080px² | 225 |
| Mount Everest | 91,810px | 233 | `<img>` 55,296px² | 429 |

#### 연기 테스트 2회가 설계를 뒤집었다

[C7 의 "캐시를 데운 뒤 잰다"](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다) 를 그대로 가져왔더니 **FCP = LCP 가 또 나왔다.**
캐시가 따뜻하면 **이미지도 캐시돼 있어서** 첫 컨텐츠 프레임에 같이 그려진다.

> 문서 사이트에서 네트워크는 **잡음**이었다. 이미지 LCP 에서는 **현상 그 자체**다.

데이터 수집 전에 캐시 상태를 축으로 올렸다 — `warm` / `cold`(빈 프로필 첫 로드). 72회.

#### 결과 — 렌더링 일도 줄고, LCP 도 빨라졌다

콜드 캐시 · ms · 중앙값 · 6회

| 사이트 | **Layout** | **FCP** | **LCP** | LCP−FCP |
|---|---|---|---|---:|
| Grand Canyon | 478 → 125ms (**−74%**) | 1,893 → 1,305ms (−31%) | 2,224 → 1,597ms (**−28%**) | 331 → 292 |
| Milky Way | 618 → 217ms (**−65%**) | 2,183 → 1,642ms (−25%) | 2,349 → 1,935ms (**−18%**) | 167 → 293 |
| Mount Everest | 644 → 312ms (**−52%**) | 2,230 → 1,810ms (−19%) | 2,346 → 2,064ms (**−12%**) | 117 → 254 |

**C7 에서 못 쟀던 FCP/LCP 분리가 처음 나왔다** (117~331ms). LCP 요소는 72회 전부 `<img>` 로 유지됐다.

#### 그런데 다운로드가 빨라진 게 아니다

LCP 이미지의 리소스 타이밍을 직접 봤다.

| 사이트 | 모드 | 요청 시작 | 전송 시간 | 바이트 | LCP |
|---|---|---:|---:|---:|---:|
| Grand Canyon | `plain` | 1,835 | 276ms | 38,064 | 2,224 |
| | `cv-auto` | **1,256** | 120ms | 38,064 | **1,597** |
| Milky Way | `plain` | 2,094 | 127ms | 19,938 | 2,349 |
| | `cv-auto` | **1,560** | 114ms | 19,938 | **1,935** |
| Mount Everest | `plain` | 2,114 | 112ms | 28,194 | 2,346 |
| | `cv-auto` | **1,699** | 127ms | 28,194 | **2,064** |

**바이트가 같다. 전송 시간도 같다.** 달라진 건 **요청 시작 시각**뿐 — 400~580ms 앞당겨졌다.

#### 원인 — LCP 이미지가 `loading="lazy"` 였다

두 번째 가설(*"cv 가 화면 밖 이미지 요청을 줄여 경쟁이 준다"*)도 틀렸다.
요청된 이미지 수와 바이트가 **두 모드 완전히 동일**했다 (10/10 · 6/6 · 18/18, 110·81·87KB).

> `loading="lazy"` 이미지는 **"뷰포트 안인가"** 를 알아야 요청이 나간다.
> 그건 **레이아웃을 해야 안다.** 레이아웃이 요청의 선행 조건이다.

#### 반증 테스트 — `lazy` 를 `eager` 로 바꿔봤다

HTML 을 가로챌 때 `loading="lazy"` 를 전부 `loading="eager"` 로 치환하고 60회 더 쟀다.
**LCP 로는 2대1 로 갈렸지만**(흩어짐이 효과 크기와 비슷해 판정이 안 된다),
**가설이 직접 예측하는 양**으로 보면 깨끗하다.

| 이미지 | 점 | Layout 절감 | **요청 앞당김** |
|---|---:|---|---|
| `lazy` | 6 (두 실행) | 57~402ms | **+59 ~ +579ms — 전부 양수** |
| `eager` | 3 | 65~134ms | **평균 −52ms** |

> **같은 레이아웃 절감이어도, 요청이 앞당겨지는 건 이미지가 `lazy` 일 때뿐이다.**

`Layout` 절감 자체는 `lazy`(59~66%)든 `eager`(61~72%)든 같았다.
달라지는 건 **그 절감이 LCP 로 이어지는 경로**다.

곁가지로: `eager` 는 요청을 300~400ms 앞당기지만 LCP 는 2~23% 만 줄었다.
화면 밖 이미지 60여 개가 한꺼번에 경쟁해 전송이 123 → 303ms 로 길어진다. **`eager` 는 공짜가 아니다.**

#### eager 가 '실제'인 사이트에서 다시 — 기전은 맞고, 이득은 없다

위 `eager` 는 내가 HTML 을 고쳐 만든 조건이다. **실제로 그런 사이트**를 찾아 18곳을 뒤졌다.
...
**Wikivoyage 는 Wikipedia 와 같은 MediaWiki 이고 선택자도 같은데 히어로 배너에만 `lazy` 가 안 붙는다.**
나머지는 대부분 lazy — 요즘 권장되는 패턴 그대로다. 같은 실행 안에 강제-lazy 대조군을 두고 60회 쟀다.

**LCP 로는 예측과 반대로 보였다.** 그런데 그건 결과가 아니라 **검정력이 없다는 뜻**이었다:

| | |
|---|---|
| 기전이 쓸 수 있는 예산 (Layout 절감) | **30~70ms** (C8 본측정은 330~400ms) |
| 요청 시작의 셀 내 MAD 중앙값 | **83ms** → 추정 표준편차 122ms |
| 검정력 0.8 에 필요한 반복 | **셀당 약 96회** — 실제로는 5회 |

시간 대신 **순서**로 재니 깨끗했다. 기전이 예측하는 건 시간 차이가 아니라 순서다:

| LCP 이미지 | 요청 전에 끝난 Layout | 회수 |
|---|---|---|
| `eager` (Wikivoyage 3곳) | **0개** / 71~137 | 6/6 |
| `lazy` (강제) | **4~8개** / 54~135 | 6/6 |
| `lazy` (Wikipedia, 실제 · 양성 대조) | **6~8개** / 54 | 2/2 |

**14회 전부 갈렸다. 중간이 없다.** 네트워크가 빠르든 느리든 부호가 안 바뀐다.

#### 비싼데 `eager` 인 페이지도 찾아서 — 네 칸을 다 채웠다

Wikivoyage 3곳은 ①도 ②도 아니었다. 두 조건을 분리하려면 **②만** 만족하는 페이지가 필요하다.
`Special:LongPages` 로 최장 문서부터 훑고, 판정은 프록시가 아니라 **실측 Layout ms** 로 했다.

| 문서 | 문서 높이 | DOM | **Layout** | LCP |
|---|---:|---:|---:|---|
| Turku | 124,362px | 15,064 | **1,143ms** | 이미지 · `eager` |
| United Kingdom | 138,175px | 6,605 | **361ms** | 이미지 · `eager` |
| Philippines | 132,372px | 6,711 | **377ms** | 이미지 · `eager` |
...
LCP 도 −3% · −12% · +1% 로 안 움직인다.

| | 레이아웃 **쌈** | 레이아웃 **비쌈** |
|---|---|---|
| **`eager`** | 앞당김 **−1~+97ms** | 앞당김 **−161~+185ms** |
| **`lazy`** | 앞당김 **+28~+149ms** | 앞당김 **+414~+579ms** |

**앞당김이 크고 일관된 칸은 오른쪽 아래 하나뿐이다.**

> **그래서 문구는 이렇게 바뀐다:**
> *"`content-visibility` 는 이미지 LCP 에는 도움이 안 된다"* →
> **"앞당기려면 둘 다 필요하다 — ① LCP 이미지가 `loading="lazy"` 이고
> ② 레이아웃 비용이 수백 ms 일 것. 다운로드를 빠르게 하는 게 아니라
> lazy 이미지의 요청이 첫 레이아웃을 기다린다는 사실을 이용하는 것이기 때문이다."**
> C8 의 Wikipedia 3곳은 둘 다 만족했다(lazy · 478~643ms). Wikivoyage 도시 3곳은 **①도 ②도 아니고**,
> Turku·UK·Philippines 는 **②만** 만족한다 — 그래서 아무 일도 안 일어난다.

곁가지: `Special:LongPages` 상위 14개 중 **이미지 LCP 는 5개뿐**이고 나머지는 `<p>` 였다.
**긴 문서일수록 텍스트가 LCP 가 되기 쉽다** — ①과 ②가 같이 성립하기 어려운 구조적 이유다.

#### 남은 735ms 도 쪼갰다

이미지가 다 내려온 뒤 LCP 까지 742~1,047ms 를 더 기다리는데, `Layout` 을 절반으로 줄여도
그 대기가 5~10% 밖에 안 줄었다. 매 프레임 배너의 크기와 `complete` 를 찍어 쪼갰다.

| 구간 | 중앙값 | 비중 |
|---|---:|---:|
| ① 응답끝 → 배너가 최종 크기로 레이아웃되기까지 | 122ms | **22%** |
| ② 그 시점 → `complete=true` | 0ms | 0% |
| ③ **준비된 뒤 실제로 그려지기까지** | **426ms** | **78%** |

**따로 잰 두 값이 맞아떨어진다.** 레이아웃 몫이 22% 뿐이니 `Layout` 을 절반으로 줄여도
대기는 최대 **11%** 밖에 못 준다 — 실측이 5~10% 였다.

뜻밖의 것도 나왔다. **LCP 후보가 갈아치워진다:**

| 시각 | 요소 | 크기 |
|---:|---|---:|
| 956ms | `<img>` 워드마크 | 3,000px² |
| 1,092ms | **`<p>`** | 83,190px² |
| 1,448ms | `<img>` 페이지 배너 ← 최종 LCP | 85,801px² |

**최종 LCP 는 이미 그려져 있던 `<p>` 를 3.1% 차이로 이긴 것이다.** Philippines 는 **1.3%** 차이다.
한 요소가 늦게 그려지는 게 아니라 **누가 제일 큰지가 늦게 결정된다.**

#### 그 426ms 도 쪼갰다

단계 이름을 추측하지 않고, **LCP 이미지의 파일명 토큰으로 트레이스 10만 개의 `args` 를 통째로 훑었다.**
그 배너에 일어난 일이 시간순으로 줄지어 나온다.

```
 1653.0ms  ResourceSendRequest
 1827.8ms  ResourceReceiveResponse
 2373.6ms  (배너가 최종 크기 + complete)
 2762.1ms  PaintImage                    ← 934ms 동안 배너 관련 이벤트가 하나도 없다
 2891.0ms  LargestImagePaint::Candidate  ← LCP
```

| 구간 | 값 |
|---|---|
| **A** 준비 → 배너의 `PaintImage` | 528 ~ 2,411ms — **전부를 좌우** |
| **B** `PaintImage` → LCP 후보 | **중앙값 117ms** · cv 를 걸어도 안 변한다(136→117 · 87→87) |

**디코드는 범인이 아니다** — `Decode Image` 가 `PaintImage` 보다 먼저 온 행이 **0/12**.
디코드는 페인트가 **부른 뒤에** 일어난다.

**왜 A 가 긴가 — 페인트가 드물다.** 같은 창 안에서:
...
**레이아웃은 50번 도는데 페인트는 9번뿐이다.** 배너는 준비돼 있어도 다음 페인트 패스까지 기다린다.
시간 합계로는 `Paint` 가 10~26ms 라 눈에 안 띈다 — [횟수로 봐야 보인다](#방법론-23조).

#### 그리고 A 를 셀당 20회로 다시 쟀다 — 바닥이 나왔다

| 지표 | United Kingdom | Philippines |
|---|---|---|
| **A** | **−420ms** [−623, −140] ★ | **−506ms** [−631, −249] ★ |
| `LCP − 응답끝` | **−320ms** [−584, −57] ★ | **−358ms** [−607, −229] ★ |
| **LCP (절대)** | −144ms [−616, 248] | −84ms [−636, 252] |

**실행 안에서 짝지어지는 지표는 전부 유의하고, 절대 LCP 만 0 을 포함한다.**

왜 A 가 주는가: **메인 스레드가 99~100% 차 있고** 그 안에서 `Layout` 이 646~669ms 를 먹는다.
cv 가 그걸 188~204ms 로 줄이면 **페인트 한 번에 걸리는 시간이 139~145ms → 86~88ms** 로 좁아진다.
`Layout` **횟수**(35회)는 그대로다 — **cv 가 줄이는 건 횟수가 아니라 한 번의 비용**이다.

그 `Layout` 35회 중 **17~19회는 화면에 아무것도 안 남긴다** — `ParseHTML` 태스크 11~13회,
`FireIdleCallback` 6회. 스트리밍 파서가 HTML 도착마다 레이아웃을 돌리는데 그 태스크는 페인트로 안 간다.
(처음엔 "JS 강제 리플로가 많을 것" 이라고 봤다가 **12회 전부 정확히 6회**로 반증됐다.)

#### 그런데 절대 LCP 는 왜 안 움직이나 — 두 번 틀리고 두 번 고쳤다

**첫 번째 설명(틀림)**: *"다운로드가 129~142ms 늦어져 상쇄된다."* 중앙값만 보고 한 말이다.
신뢰구간을 붙이니 0 을 포함하고, 독립 실행에서 **반대 부호**가 나왔다. 두 실행을 합치면 +33·+61ms —
**다운로드는 cv 와 무관하다**(우선순위 24회 전부 `Medium`, 승급 0/24, 경쟁 바이트 동일).

**두 번째 설명(반만 맞음)**: *"검정력이다."* 맞다 — 콜드에서 −144ms 를 잡으려면 셀당 **약 180회**가 필요하다.
그래서 캐시를 데워 네트워크 변동을 없애고 80회 더 쟀다. **그래도 절대 LCP 는 0 을 포함했다.**

**진짜 이유**: warm 에서 **cv 가 첫 페인트를 늦춘다.** 세 실험 40회/셀을 합치면:

| 사이트 | `plain` FCP | `cv-auto` FCP | 차이 |
|---|---:|---:|---|
| United Kingdom | 372ms | 620ms | **+248** [180, 344] ★ |
| Philippines | 384ms | 544ms | **+160** [40, 276] ★ |

FCP 이전 구간만 잘라 보면 어디서 쓰는지 나온다:
...
#### 그 선행 비용의 일부는 `auto` 키워드다

`전Layout` 이 양봉 분포라 중앙값으로 뭉개면 안 된다. **횟수로 셌다:**

| 조건 | `FCP` 이전 `Layout` > 50ms 인 실행 |
|---|---|
| `plain` | 4/40 (10%) |
| `cv-auto` | **30/40 (75%)** — p < 0.0001 |
| `cv-fixed` <sub>`auto` 키워드만 뺌</sub> | **9/20 (45%)** — cv-auto 대비 **p = 0.040** |

[C2 는 `auto 73px` 과 `73px` 의 차이가 3~8% 라고 봤다](#c2--같은-css-가-조건에-따라-손해를-본다) —
**그건 리레이아웃 이야기였다.** 첫 페인트 전에는 비용이 나는 빈도가 75% → 45% 로 갈린다.
(p=0.040, `cv-fixed` 20회 — **약한 증거**다.)

#### 적게 걸면 되나 — 절반만

요소마다 붙는 비용이면 개수에 비례해야 한다. **용량-반응으로 쟀다**(80회).
`nth-of-type` 은 중첩 섹션마다 걸리므로 K 가 곧 개수가 아니다 — 실제 매칭 개수를 실행마다 기록했다.

| cv 요소 | UK 전`Layout` | Philippines 전`Layout` |
|---:|---:|---:|
| 0 (`plain`) | **1ms** | **1ms** |
| 252~373 | 46ms | 66ms |
| 468~588 | 118ms | 128ms |
...
**고정 비용은 아니다** — 개수와 함께 커진다.

#### 그런데 그 "2.8배" 는 내 지표가 만든 것이었다

요소당 비용을 **분자는 `FCP` 까지, 분모는 끝까지 파싱된 개수**로 계산하고 있었다.
FCP 가 이른 실행에서는 분자가 ≈1ms 가 된다. 기존 데이터로 바로 드러났다:
...
기준선을 **DOMContentLoaded**(두 팔 모두 문서를 끝까지 파싱한 지점)로 바꿔 80회 다시 쟀다.

| cv 요소 | UK `Layout` | 요소당 | Philippines `Layout` | 요소당 |
|---:|---:|---:|---:|---:|
| 0 | 37ms | | 39ms | |
| 252~373 | 45 (**+9**) | 23µs | 53 (**+15**) | 58µs |
| 468~588 | 53 (+16) | 27µs | 58 (+19) | 41µs |
| 636~743 | 55 (+19) | 25µs | 59 (+20) | 32µs |
| 822~865 | 62 (**+25**) | 29µs | 77 (**+38**) | 47µs |

`ParseHTML` 은 **13~15ms 로 전 팔 동일**했다. `Layout` 은 두 사이트 모두 **예외 없이 단조 증가**한다.

**요소당 비용은 23~58µs — 아까의 10분의 1이고, 사이트 안에서 1.26~1.81배로 2배 이내다.**
사이트 간 차이(18µs)는 신뢰구간이 0 을 포함한다.

> **C8m 에서 반증했다고 쓴 두 주장이 여기서 뒤집혔다.**
> "적게 걸면 비용이 거의 안 난다"(+9·+15ms)도, "요소당 비용이 2배 이내"도 **원래 예측이 맞았다.**
> 예측이 틀린 게 아니라 **내 측정이 틀렸다.**

#### 대신 더 나은 것이 나왔다 — cv 는 첫 페인트에 바닥을 깐다
...
**`plain` 은 레이아웃을 거의 안 한 채로 먼저 그리고, cv 는 골격을 세운 뒤에야 그린다.**

네 실험을 늘어놓으면 지연이 **기준선에 반비례**한다:
...
> **r(기준선 FCP, 지연) = −0.88.** 풀링하면(48/셀) **UK +264 [180, 320] · Philippines +248 [136, 320]**, 둘 다 유의하다.

#### 사이트를 8곳으로 늘리니 "바닥" 이라는 말이 틀렸다

FCP 는 LCP 와 무관하므로 "이미지 LCP" 조건을 빼고, **DOM 수와 블록 수가 안 묶인 8곳**을 골랐다(r = 0.53).
DOM 1,398~15,413 · 블록 96~973. 80회.

| 예측자 | cv FCP | plain FCP |
|---|---|---|
| **`DOMContentLoaded`** | **0.99** [0.98, 0.99] | **0.97** [0.96, 0.99] |
| DOM 수 | 0.32 [−0.06, 0.64] | 0.31 |
| 블록 수(cv 요소 수) | **0.11** [−0.29, 0.34] | −0.00 |

**cv 요소 수는 아무것도 설명하지 못한다.** FCP 는 두 모드 모두 **파싱 완료를 따라간다.**
다른 건 *얼마나 앞서 그리느냐* 다:

| 사이트 | DOM | plain | cv | 차이 |
|---|---:|---:|---:|---:|
| Interstate 5 | 1,398 | 150 | 98 | −52 |
| EuroVelo 10 | 3,712 | 253 | 221 | −32 |
| United Kingdom | 6,605 | 710 | 413 | −297 |
| Buffalo/East Side | 12,517 | 644 | 242 | **−402** |
| London | 15,413 | 787 | 432 | −355 |
| **중앙값(8곳)** | | **644** | **363** | **−186** |

*(DCL 보다 몇 ms 먼저 그리는가)*

> **8곳 전부 줄었다.** 행 단위로 554 → 285ms, 차이 **−269ms [−436, −20] ★**
> **바닥이 아니라 머리 시작을 깎는 것이다.** cv 가 남기는 머리 시작은 `plain` 의 **약 60%**.
> 그리고 머리 시작 자체가 DOM 에 비례해 커지므로(150 → 787ms) 감소폭도 커진다 — **r(DOM, 감소폭) = −0.90**.

*"머리 시작이 그냥 DCL 에 비례하는 것 아닌가"* 도 확인했다. **아니다** —
r(DOM, 머리 시작) = **0.74** vs r(DCL, 머리 시작) = 0.41 이고,
FCP÷DCL 비율 차이는 +0.081 **[−0.002, 0.166] 로 0 을 포함**한다("고정 비율" 기각).
문서가 클수록 그릴 거리가 일찍 쌓여 머리 시작이 길어지는 것으로 읽힌다.

C8n 에서 cv FCP 가 안정적으로 보인 건 **같은 2곳을 여러 세션에서 봤기 때문**이다.
8곳을 한 세션에서 보면 cv FCP 범위가 오히려 더 넓다(3.5배 vs 2.9배).
**세션 간 변동과 사이트 간 변동은 다른 축이다.**

두 독립 추정이 맞아떨어진다 — warm FCP 차이(2곳 × 4세션) **+248~264ms**,
DCL 머리 시작 차이(8곳 × 1세션) **+269ms**.

> **최종:** `content-visibility` 는 렌더링 대기를 확실히 줄인다(−320~−506ms, 신뢰구간이 0 제외).
> 그런데 그게 **절대 LCP 로는 안 나타난다** — 콜드에서는 네트워크 변동(±600ms)에 묻히고,
> warm 에서는 **cv 자신이 첫 페인트를 160~248ms 늦춰서** 상쇄된다.
> 그 선행 비용은 cv 요소 수에 **거의 비례**하고(요소당 23~58µs), 적게 걸면 그만큼 줄어든다.

[C7 의 결론](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다)과 같은 모양이다 —
렌더링 일이 줄어도 **사용자에게 보이려면 그 일이 병목이어야 한다.**

<sub>사전 등록 [PREDICTION.md](experiments/track-c/c8-image-lcp/PREDICTION.md) · 전체 결과 [RESULTS.md](experiments/track-c/c8-image-lcp/RESULTS.md) — 결과 파일에 남은 측정 **844행** · 사전 등록 주장 38개 중 **9 반증 · 20 성립 · 6 부분 · 4 판정 불가**</sub>
...
| "`contain-intrinsic-size` 는 값만 잘 주면 된다" | ⚠️ **실제 페이지에선 줄 수 있는 값이 없다.** 오차가 블록 높이 변동계수를 따라간다 — 실측 3곳에서 **32% · 47% · 89%** |
| "`content-visibility` 는 아무 페이지에나 붙일 수 있다" | ❌ 반복 단위가 `<tr>` 이면 **`contain: size` 가 적용 안 된다.** 조사한 9곳 중 쓸 만한 건 3곳 |
| "`content-visibility` 는 조건에 따라 손해다" | ❌ 실제 사이트 **3곳 어디서도 손해가 없었다** (5.9~11.3배 이득, 최악도 무승부). 합성 하네스가 손해를 만들어냈다 |
| "`content-visibility` 는 초기 렌더를 빠르게 한다" | ⚠️ **레이아웃이 병목일 때만.** 렌더링 일은 어디서나 84~99% 줄지만 LCP 는 3곳 중 **1곳**에서만 움직였다 (24초 → 2.2초) |
| "LCP 가 이미지면 `content-visibility` 는 소용없다 (다운로드가 병목이니까)" | ❌ **12~38% 빨라진다.** 단 다운로드가 빨라지는 게 아니다 — 바이트·전송시간이 똑같고 **요청 시작만** 400~580ms 앞당겨진다 |
| "그건 화면 밖 이미지를 안 받아서 그렇다" | ❌ **요청 수·바이트가 두 모드 완전히 동일.** 원인은 LCP 이미지가 `loading="lazy"` 라 **레이아웃이 요청의 선행 조건**이라는 것 |
| "`loading=\"eager\"` 로 바꾸면 LCP 가 그만큼 빨라진다" | ⚠️ 요청은 300~400ms 앞당겨지는데 LCP 는 2~23% 만. 화면 밖 이미지가 한꺼번에 경쟁해 **전송이 123 → 303ms** 로 길어진다 |
| "그럼 `content-visibility` 를 붙이면 이미지 LCP 가 빨라지겠네" | ⚠️ **조건이 둘 다 필요하다** — ① LCP 이미지가 `lazy` ② 레이아웃이 수백 ms. 실제 `eager` 사이트 3곳에서는 요청이 **첫 레이아웃 전에** 나가고(14/14) 레이아웃 예산도 30~70ms 라 아낄 게 없다 |
| "레이아웃만 충분히 비싸면 이미지 LCP 도 빨라지겠지" | ❌ **아니다.** Turku 는 레이아웃을 **424ms** 아꼈는데(Canyon 의 354ms 보다 많다) 요청이 **38ms 밀렸다.** LCP 이미지가 `eager` 라 요청이 레이아웃을 안 기다린다 |
| "LCP 는 그 이미지가 준비된 시각과 비슷할 것" | ❌ 준비된 뒤 **426ms**(78%)를 더 기다린다. 그 동안 프레임은 나오지만 느리다. 레이아웃 몫은 **22%** 뿐이라 `content-visibility` 로는 최대 11% 밖에 못 줄인다 |
| "늦는 건 큰 이미지 디코드 때문이겠지" | ❌ `Decode Image` 가 `PaintImage` **보다 먼저 온 적이 0/12**. 디코드는 페인트가 **부른 뒤에** 일어난다 |
| "`content-visibility` 는 첫 페인트를 늦출 리 없다 (화면 밖을 건너뛰니까)" | ❌ **160~248ms 늦춘다**(warm, 40회/셀, 신뢰구간이 0 제외). 화면 밖을 건너뛰는 대신 **첫 페인트 전에 전체 골격을 한 번 세운다** — 첫 페인트 전 `Layout` 이 1ms → 200ms |
| "늦추는 폭은 cv 를 건 요소 수가 정한다" | ❌ **cv 요소 수는 아무것도 설명 못 한다**(r = 0.11). FCP 는 두 모드 다 **파싱 완료를 따라가고**(r = 0.97~0.99), cv 는 그보다 **먼저 그리는 폭을 60% 로 깎는다**(8곳 전부, −269ms) |
| "그럼 화면 아래쪽에만 조금 걸면 되겠네" | ✅ **줄어든다.** DCL 기준 요소당 **23~58µs**, 865개 +25ms → 373개 +9ms 로 거의 비례해서 준다. (처음엔 "비례하지 않는다" 고 썼는데 [내 지표 탓이었다](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다)) |
| "`contain-intrinsic-size` 의 `auto` 는 성능에 거의 영향이 없다" | ⚠️ [리레이아웃에서는 3~8%](#c2--같은-css-가-조건에-따라-손해를-본다). **첫 페인트 전에는 다르다** — 선행 비용이 나는 빈도가 `auto` 75% vs 고정 45%(p=0.040, 약한 증거) |
...
| 16 | "항목 스타일만 바뀌면 cv 가 4.5배 손해" | C6 실제 사이트 | **합성 한정.** 실제 페이지에서는 **무승부**(1.01배) |
| 17 | "실제 페이지가 합성보다 cv 에 불리할 것" | C6 (사전 등록 Q2·Q3) | **전부 유리한 쪽으로 틀렸다.** 내 편향이 일관돼 있었다 |
| 18 | "cv 의 이득은 Style 에서 나온다" (C2 이후 계속) | C6 실제 3곳 | **Layout 이다.** 폭 변경에서 3곳 모두 큰 이득인데 Style 은 2곳에서 **증가**한다 |
| 19 | "cv 는 초기 렌더를 빠르게 한다 (LCP −36~39%)" | C7 실제 3곳 | **레이아웃이 병목일 때만.** 3곳 중 2곳은 **차이 없음** |
| 20 | "이미지가 LCP 면 다운로드가 병목이라 cv 로 안 빨라진다" | C8 (사전 등록 S2·S6) | **12~38% 빨라진다.** 다만 다운로드가 빨라진 게 아니다 |
| 21 | "cv 가 화면 밖 이미지 요청을 줄여서 빨라진 것" | C8 `probe-why` | **요청 수·바이트가 완전히 동일.** 원인은 `loading="lazy"` 였다 |
...
20. **내가 만든 안전장치를 내 손으로 치우지 않는다**
21. **최종 지표가 아니라 가설이 예측하는 양을 잰다**
22. **절대값 대신 실행 안에서 짝지어지는 값을 쓴다**
23. **비율을 만들 때 분자와 분모의 기준선을 맞춘다**
...
| 20 | [C7](#c7--초기-로드의-실제판--lcp-3639-단축-은-조건부였다) — 중복 실행 락에 PID 검사를 넣어놓고 **내가 `rm` 으로 치웠다.** 36건 중 24건을 날렸다 |
| 21 | [C8](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다) — 반증 테스트를 **LCP 로 판정하려니 2대1 로 갈렸다.** 흩어짐(1.3~1.6배)이 효과 크기와 비슷했다. 가설이 직접 예측하는 양(**요청 시작 시각**)으로 보니 3대0 이었다 |
| 22 | [C8](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다) — 절대 LCP 는 80회를 돌려도 신뢰구간이 0 을 포함했다. 같은 데이터에서 **`LCP − 응답끝`** 은 유의했다. 네트워크 변동이 통째로 들어가는 값 대신 **한 실행 안의 두 시각 차이**를 쓰면 잡음이 빠진다 |
| 23 | [C8](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다) — "요소당 비용" 의 분자는 `FCP` 까지, 분모는 **끝까지 파싱된 개수**였다. 기준이 어긋나니 2.8배(중앙값)·669배(행 단위)가 나왔다. 기준선을 `DOMContentLoaded` 로 맞추자 **2배 이내**가 됐다 |
| 22 | "cv 가 LCP 를 못 움직인 건 다운로드가 129~142ms 늦어져 상쇄돼서다" | C8i 독립 24회 | **중앙값만 보고 만든 이야기.** 신뢰구간이 0 을 포함하고 다른 사이트는 **반대 부호**였다 |
| 23 | "A 구간의 레이아웃은 JS 강제 리플로가 많을 것" | C8h 12회 | **정확히 6회뿐**이었고 전부 `FireIdleCallback`. 나머지는 **`ParseHTML` 태스크**였다 |
| 24 | "cv 의 요소당 비용이 2.8배 흔들린다" | C8n DCL 기준 80회 | **내 지표가 만든 값.** 분자는 FCP 까지, 분모는 끝까지였다. 제대로 재면 **23~58µs, 2배 이내** |
| 25 | "cv 는 첫 페인트에 **바닥**을 깐다" | C8o 8곳 80회 | **바닥이 아니다.** FCP 는 두 모드 다 파싱 완료를 따라가고(r=0.99), cv 는 **먼저 그리는 폭을 60% 로 깎는다** |
...
│   ├── c6-real-sites/           PREDICTION · probe ×4 (structure/target/granularity/candidates)
│   │                            measure(3사이트) · analyze · analyze-all · RESULTS
│   ├── c7-real-initial-load/    PREDICTION · measure · analyze · recover · RESULTS
│   └── c8-image-lcp/           PREDICTION · probe-lcp · probe-target · probe-why
│                               measure · measure-lazy · analyze · analyze-lazy · RESULTS
│
└── report/
...
| ~~이미지가 LCP 인 페이지에서는 결론이 다른가~~ | ✅ **닫음** → [C8](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다). 빨라진다. 단 경로가 `loading="lazy"` 를 거친다 |
| **`cv-auto` 의 cold Layout 절감(52~65%)이 warm(82~90%)보다 작은 이유** | ❌ 미확인. 이미지 도착마다 리레이아웃이 도는 것으로 보이는데 계수하지 않았다 |
| ~~LCP 이미지가 `eager` 인 실제 사이트에서도 같은가~~ | ✅ **닫음** → C8c. 기전은 확인(순서 14/14), 이득은 없다 |
| ~~레이아웃이 비싼데 LCP 이미지가 `eager` 인 페이지~~ | ✅ **닫음** → C8d. Turku(876ms)·UK·Philippines. **네 칸이 다 찼다** |
| ~~이미지가 다 내려온 뒤 LCP 까지의 735ms~~ | ✅ **쪼갬** → C8e. 레이아웃 **22%** · 준비된 뒤 그려지기까지 **78%**. 두 독립 측정이 맞아떨어진다 |
| ~~그 78%(426ms)가 파이프라인 어느 단계인가~~ | ✅ **쪼갬** → C8f. **A**(다음 페인트 패스까지 기다림) + **B**(고정 117ms). 디코드는 아니다(0/12) |
| ~~A 를 무엇이 밀고 있나~~ | ✅ **닫음** → C8g·C8h. 메인 스레드 99~100% 점유, `Layout` 646~669ms. cv 가 줄이면 페인트 간격이 139→87ms |
| ~~절대 LCP 가 왜 안 움직이나~~ | ✅ **닫음** → C8i·C8j·C8k. 콜드는 네트워크 변동, warm 은 cv 자신의 선행 비용(FCP +160~248ms) |
| **cv 의 선행 비용을 피할 수 있나** | ❌ 화면 아래쪽에만 거는 CSS 가 없다. `auto` 를 빼면 75%→45% 지만 스크롤바 정확성을 잃는다 |
| ~~cv 가 첫 페인트에 까는 바닥은 무엇이 정하나~~ | ✅ **닫음** → C8o. **바닥이 아니었다.** 머리 시작을 60% 로 깎는 것 |
| **머리 시작이 왜 DOM 에 비례하나** | ❌ r(DOM, 감소폭) = −0.90 까지는 봤지만, 문서 높이가 블록 수와 r = 0.93 이라 8곳으로는 못 가른다 |
| **필드 데이터(p75)에서는 어느 쪽이 맞나** | ❌ 미검증. 실험실에서는 실행 내 지표가 유의하고 절대 LCP 는 아니다. 실사용자 분포는 못 봤다 |
| 위키 밖의 이미지 LCP 페이지 | ❌ 18곳 중 0곳. 긴 문서는 텍스트 LCP, 이미지 LCP 는 문서가 짧고, 나머지는 봇 차단 |
