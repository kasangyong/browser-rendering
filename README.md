...
인터랙티브 데모 **10개** · 실험 **12개** · 사전 등록 주장 **37개**(20 성립 · 13 반증 · 4 부분·보류)

**내가 틀린 것 9개** · 폐기한 측정법 **5개**
...
**왜 안 켜지는지는 모른다.** 그럴듯한 설명을 하나 더 지어내는 대신 열어뒀다 —
방금 지어낸 가림 가설이 바로 그래서 틀렸다. (주 결론은 위 세 갈래 증거에 기대고 있어 영향받지 않는다.)
...
│   └── c4-raster-threshold/     PREDICTION · measure · probe-settle · probe-why · RESULTS
│
└── report/
...
| **큰 박스(100×100)는 왜 한 번도 안 켜지나** | ❌ 미확인. 초기 래스터가 **더 오래** 걸리는데 측정 창으로는 안 샌다. 가림 가설은 실측으로 기각 |
