...
인터랙티브 데모 **10개** · 실험 **31개** · 사전 등록 주장 **94개**(46 성립 · 33 반증 · 15 부분·보류) · **실측 사이트 12곳**
...
시간 합계로는 `Paint` 가 10~26ms 라 눈에 안 띈다 — [횟수로 봐야 보인다](#방법론-22조).
...
**고정 비용은 아니다** — 개수와 함께 커진다. 그런데 **관계가 약하다**(순위상관 rho = 0.24 · 0.31).
요소당 비용은 **121~332µs 로 2.8배** 흔들린다("2배 이내" 로 예측했다가 틀렸다).

그리고 **가장 작게 건 팔(252~373개)이 이미 46~66ms 를 쓴다.**
*"몇 개로는 비용이 안 난다"* 는 예측도 틀렸다.
...
> 그 선행 비용은 cv 요소 수와 함께 커지지만 비례하지 않고, **적게 걸어도 46~66ms 는 낸다.**
...
| "그럼 화면 아래쪽에만 조금 걸면 되겠네" | ⚠️ **절반만 맞다.** 적게 걸면 273→66ms 로 줄지만 **비례하지 않고**(순위상관 0.24~0.31), 가장 작게 건 팔(252~373개)도 **46~66ms** 를 낸다 |
