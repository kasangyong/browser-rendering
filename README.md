...
인터랙티브 데모 **10개** · 실험 **25개** · 사전 등록 주장 **80개**(38 성립 · 29 반증 · 13 부분·보류) · **실측 사이트 12곳**
...
시간 합계로는 `Paint` 가 10~26ms 라 눈에 안 띈다 — [횟수로 봐야 보인다](#방법론-21조).

A 를 무엇이 밀고 있는지는 **못 닫았다.** 방향은 레이아웃 쪽(−23~−36%)인데 셀당 3회로는 부족하고,
[C8d 는 6회로 LCP 가 안 움직인다](#c8--lcp-가-이미지일-때--빨라지긴-하는데-이유가-달랐다)고 봤다.
...
| **A 를 무엇이 밀고 있나** | ❌ 방향은 레이아웃 쪽(−23~−36%)인데 셀당 3회로는 못 닫는다. C8d 의 6회 결과와 화해시키려면 더 재야 한다 |
