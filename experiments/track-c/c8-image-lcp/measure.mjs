 ...
 * C8 — LCP 가 이미지일 때
 *
 * C7 의 3곳은 전부 FCP = LCP 였다(첫 페인트에 나온 텍스트가 그대로 LCP).
 * 그래서 사전 등록한 R3(FCP/LCP 분리)을 측정조차 못 했다.
 * 실제 웹에서 LCP 는 대개 이미지고, 이미지 LCP 는 레이아웃이 아니라 다운로드에 묶여 있다.
 *
 * 공통: HTML 응답을 가로채 <style> 을 파싱 시점에 심는다. 엄격 격리 + 순서 섞기.
 * 공개 페이지를 읽기만 한다.
 *
 * C7 과 다른 점 셋:
 *   · 뷰포트가 390×844 (모바일). 데스크톱 폭에서는 LCP 가 전부 텍스트였다 — PREDICTION.md 참고
 *   · **캐시 상태가 축이다.** C7 처럼 데우고 재면(warm) 이미지도 캐시돼서
 *     첫 컨텐츠 프레임에 같이 그려진다 — FCP=LCP 가 또 나오고 재려던 걸 못 잰다.
 *     그래서 빈 프로필 첫 로드(cold)를 같이 잰다. PREDICTION.md 부칙 참고.
 *   · **LCP 요소가 무엇이었고 언제 다 내려왔는지**를 같이 기록한다.
 *     트레이스 이벤트만으로는 요소를 알 수 없어서 PerformanceObserver 를 HTML 에 같이 심는다.
 *     (양쪽 팔에 동일하게 심으므로 공정하다)
 *
 * 사전 등록: PREDICTION.md
 * 사용: node measure.mjs [--repeat=6] [--site=all|canyon|milky|everest] [--resume=1]
 */
...
const SITES = {
  canyon:  { url: 'https://en.wikipedia.org/wiki/Grand_Canyon',
             blocks: '.mw-parser-output section > *', intrinsic: 208 },
  milky:   { url: 'https://en.wikipedia.org/wiki/Milky_Way',
             blocks: '.mw-parser-output section > *', intrinsic: 234 },
  everest: { url: 'https://en.wikipedia.org/wiki/Mount_Everest',
             blocks: '.mw-parser-output section > *', intrinsic: 200 },
};
const MODES = ['plain', 'cv-auto'];
const CACHES = ['warm', 'cold'];   // cold = 빈 프로필 첫 로드 (실제 첫 방문)
...
async function once({ port, tag, siteKey, mode, cache }) {
  ...
  try {
    ...
    return {
      ...
      lcpRes: after?.lcpRes ?? null,         // 그 이미지의 다운로드 완료 시각
      stage, probe, after,
...
const out = { experiment: 'c8-image-lcp', generatedAt: new Date().toISOString(),
...
const plan = [];
for (const k of keys) for (const m of MODES) for (const c of CACHES)
  for (let r = 0; r < REPEAT; r++) plan.push({ siteKey: k, mode: m, cache: c, rep: r });
const order = shuffled(plan, SEED);
const BASELINE = await chromeCount();
console.log(`\n이미지 LCP 측정 — 사이트 ${keys.length}곳 × 모드 ${MODES.length} × 캐시 ${CACHES.length} × ${REPEAT}회 = ${order.length}회`);
console.log(`뷰포트 ${VIEWPORT.w}×${VIEWPORT.h} (모바일) · 기준선 chrome ${BASELINE}개 · 순서 섞음\n`);

let port = 42000 + Math.floor(Math.random() * 300);
let i = 0;
for (const job of order) {
  i++;
  const key = `${job.siteKey}|${job.mode}|${job.cache}|${job.rep}`;
  if (out.rows.some(r => `${r.siteKey}|${r.mode}|${r.cache}|${r.rep}` === key)) continue;
  await waitQuiet(BASELINE);
  port++;
  try {
    const v = await once({ port, tag: `${job.siteKey}-${job.mode}-${job.cache}-${job.rep}`, ...job });
    out.rows.push({ ...v, rep: job.rep });
    await save();
    console.log(`  ${String(i).padStart(2)}/${order.length}  ${job.siteKey.padEnd(9)}${job.mode.padEnd(9)}${job.cache.padEnd(6)}` +
