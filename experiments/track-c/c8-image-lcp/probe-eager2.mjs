...
const SITES = [
  // Wikivoyage — Wikipedia 와 같은 MediaWiki 인데 히어로 이미지에 lazy 가 안 붙는다.
  // 같은 엔진·같은 선택자·같은 측정에서 loading 만 다른 대조군이 된다.
  { key: 'wv-paris',  url: 'https://en.wikivoyage.org/wiki/Paris',  sel: '.mw-parser-output section > *' },
  { key: 'wv-rome',   url: 'https://en.wikivoyage.org/wiki/Rome',   sel: '.mw-parser-output section > *' },
  { key: 'wv-tokyo',  url: 'https://en.wikivoyage.org/wiki/Tokyo',  sel: '.mw-parser-output section > *' },
  { key: 'wv-london', url: 'https://en.wikivoyage.org/wiki/London', sel: '.mw-parser-output section > *' },
  { key: 'wv-nyc',    url: 'https://en.wikivoyage.org/wiki/New_York_City', sel: '.mw-parser-output section > *' },
  // 위키 밖에서도 하나는 있어야 외적 타당성이 선다
  { key: 'seriouseats', url: 'https://www.seriouseats.com/food-lab-complete-guide-to-sous-vide-steak' },
  { key: 'dpreview',    url: 'https://www.dpreview.com/reviews/fujifilm-x-t5-review' },
  { key: 'nps-long',    url: 'https://www.nps.gov/yell/planyourvisit/things-to-do.htm' },
];
...
const REPORT = `(() => {
  ...
  // 미리 정한 선택자가 있으면 그것도 같이 잰다 (자동 탐색은 한 요소의 자식만 본다)
  let given = null;
  const GIVEN = window.__sel;
  if (GIVEN) {
    const els = [...document.querySelectorAll(GIVEN)];
    const hs = els.map(e => e.getBoundingClientRect().height).filter(h => h > 0);
    if (hs.length) {
      const srt = [...hs].sort((a, b) => a - b);
      const m = hs.reduce((a, b) => a + b, 0) / hs.length;
      const sd = Math.sqrt(hs.reduce((a, b) => a + (b - m) ** 2, 0) / hs.length);
      given = { sel: GIVEN, count: els.length, median: +srt[Math.floor(srt.length / 2)].toFixed(0),
                cv: +(sd / m).toFixed(2), covers: +(hs.reduce((a, b) => a + b, 0) / H).toFixed(2) };
    } else given = { sel: GIVEN, count: els.length, median: 0, cv: 0, covers: 0 };
  }
  return {
    given,
...
async function probe(site, port) {
  const profile = path.join(SCRATCH, `c8e2-${site.key}`);
  ...
  try {
    ...
    if (site.sel) await send('Page.addScriptToEvaluateOnNewDocument', { source: 'window.__sel=' + JSON.stringify(site.sel) });
...
for (const s of SITES) {
  ...
  try {
    ...
    const show = c => console.log(`   블록 ${String(c.count).padStart(4)}개  ${c.sel.slice(0, 32).padEnd(32)} 중앙값 ${String(c.median).padStart(5)}px  변동계수 ${c.cv}  문서의 ${(c.covers * 100).toFixed(0)}%`);
    if (r.given) { process.stdout.write('  ★'); show(r.given); }
    for (const c of r.cands) show(c);
    const best = (r.given && r.given.count >= 40) ? r.given : r.cands[0];
    const ok = isImg && notLazy && r.docHeight >= 15000 && best && best.count >= 40;
    console.log(`   → ${ok ? '✅ 쓸 수 있다' : '✗ ' + [
      !isImg && '이미지 LCP 아님', isImg && !notLazy && 'LCP 이미지가 lazy',
      r.docHeight < 15000 && '문서가 짧음', !best && '블록 컨테이너 없음'].filter(Boolean).join(' · ')}`);
  } catch (e) { console.log('   실패:', e.message); out.push({ ...s, error: e.message }); }
  await sleep(1500);
}
await writeFile(new URL('./eager-candidates-2.json', import.meta.url), JSON.stringify({
