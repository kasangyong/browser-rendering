/**
 * C8b — 반증 테스트: LCP 개선이 정말 lazy 때문인가
 *
 * C8 본 측정에서 cv 가 LCP 를 12~38% 줄였다. 그런데 다운로드가 빨라진 게 아니라
 * 요청 시작이 400~580ms 앞당겨진 것이었고, LCP 이미지는 세 곳 전부 loading="lazy" 였다.
 *
 * 가설: lazy 이미지는 "뷰포트 안인가" 를 알아야 요청이 나가고 그건 레이아웃을 해야 안다.
 *       레이아웃이 요청의 선행 조건이므로, cv 가 레이아웃을 줄이면 요청이 앞당겨진다.
 *
 * 반증 방법: HTML 을 가로챌 때 loading="lazy" 를 loading="eager" 로 바꾼다.
 *            가설이 맞으면 eager 에서 cv 의 LCP 개선이 사라져야 한다.
 *
 * 콜드 캐시만 잰다 (warm 은 이미지가 캐시돼 있어 lazy 든 eager 든 의미가 없다).
 * 2x2 (plain·cv-auto × lazy·eager) × 3곳 × 5회 = 60회.
 *
 * 사전 등록: PREDICTION.md 부칙 2 (S8~S10)
 * 사용: node measure-lazy.mjs [--repeat=5] [--site=all|canyon|milky|everest] [--resume=1]
 */
...
const LAZYS = ['lazy', 'eager'];   // eager = HTML 의 loading 속성을 통째로 바꿔치기
  ...
  try {
    ...
    page.on('Fetch.requestPaused', async p => {
      try {
        ...
        // 반증 조건: lazy 를 전부 eager 로. CSS 로는 못 바꾸므로 HTML 문자열에서 직접.
        if (lazy === 'eager') html = html.split('loading="lazy"').join('loading="eager"');
    ...
    return {
      ...
      imgLazy: after?.imgLazy ?? null,         // 그 이미지의 다운로드 완료 시각
      stage, probe, after,
...
const LOCK = path.join(import.meta.dirname, '.measure-lazy.lock');
if (existsSync(LOCK)) {
...
const OUT = path.join(import.meta.dirname, 'results-lazy.json');
const out = { experiment: 'c8b-lazy-falsification', generatedAt: new Date().toISOString(),
...
console.log(`\nC8b 반증 테스트 — 사이트 ${keys.length}곳 × 모드 ${MODES.length} × lazy/eager × ${REPEAT}회 = ${order.length}회 (콜드)`);
console.log(`뷰포트 ${VIEWPORT.w}×${VIEWPORT.h} (모바일) · 기준선 chrome ${BASELINE}개 · 순서 섞음\n`);

let port = 44000 + Math.floor(Math.random() * 300);
let i = 0;
for (const job of order) {
  ...
  try {
    ...
    console.log(`  ${String(i).padStart(2)}/${order.length}  ${job.siteKey.padEnd(9)}${job.mode.padEnd(9)}${job.lazy.padEnd(7)}` +
      ...
      `  lazy남음 ${v.after ? v.after.imgLazy + '/' + v.after.imgTotal : '?'}` +
...
await save();
console.log('\n✅ results-lazy.json 저장 —  node analyze-lazy.mjs');
process.exit(0);
