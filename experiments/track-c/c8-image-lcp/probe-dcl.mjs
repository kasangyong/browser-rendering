/**
 * C8n — 같은 파싱 지점(DCL)에서 요소당 비용을 다시 잰다
 *
 * C8m 의 "요소당 비용 2.8배" 는 내 지표가 만든 것이었다.
 * 분자(FCP 이전 Layout)는 FCP 까지만 세는데 분모(cv 요소 수)는 끝까지
 * 파싱된 뒤의 값이다. FCP 가 이르면 분자가 1ms 가 된다.
 * 실제로 r(FCP, 요소당비용) = 0.76 · 0.84 였다.
 *
 * 그래서 기준선을 DOMContentLoaded 로 바꾼다 — 두 팔 모두 문서를 끝까지
 * 파싱한 같은 지점이다. 그 구간의 Layout 을 재고 plain 을 빼서
 * 요소당 한계비용을 구한다.
 *
 * warm 캐시 · 2곳 × 5팔 × 8회 = 80회.
 *
 * 사전 등록: PREDICTION.md 부칙 8 (Y1~Y4)
 * 사용: node probe-dcl.mjs [--repeat=8] [--resume=1]
 */
...
async function once({ port, tag, siteKey, mode }) {
  ...
  const profile = path.join(SCRATCH, `c8n-${tag}`);
...
const LOCK = path.join(import.meta.dirname, '.dcl.lock');
if (existsSync(LOCK)) {
...
const OUT = path.join(import.meta.dirname, 'results-dcl.json');
const out = { experiment: 'c8n-dcl-baseline', generatedAt: new Date().toISOString(),
...
console.log(`\nC8n DCL 기준 — ${keys.length}곳 × ${MODES.length}모드 × ${REPEAT}회 = ${order.length}회`);
console.log(`기준선 chrome ${BASELINE}개 · 순서 섞음 · ${WARM ? '캐시 데운 뒤 2차 로드' : '콜드 캐시'}\n`);

let port = 40100 + Math.floor(Math.random() * 200);   // 임시 포트 범위(49152+)를 피한다
let i = 0, fail = 0;
