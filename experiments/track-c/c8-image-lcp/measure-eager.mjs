/**
 * C8c — eager 가 '실제'인 사이트에서 다시 잰다
 *
 * C8b 의 eager 는 내가 모든 이미지를 한꺼번에 eager 로 만든 조건이었다.
 * 화면 밖 이미지 60여 개가 경쟁해 전송이 123 -> 303ms 로 길어지는 부작용이 있었다.
 *
 * Wikivoyage 는 Wikipedia 와 같은 MediaWiki 인데 히어로 배너에 lazy 가 안 붙는다.
 * 나머지 이미지는 대부분 lazy 다 — 요즘 권장되는 패턴 그대로다.
 *
 * C8 과 세션이 다르므로 가로질러 비교할 수 없다(같은 조건 Layout 이 4배 갈린 전례).
 * 그래서 대조군을 같은 실행 안에 둔다:
 *   asis   — HTML 을 손대지 않는다. 히어로가 실제로 eager
 *   forced — loading 속성이 없는 img 에 loading="lazy" 를 붙인다
 *
 * 3곳 x 2모드 x 2팔 x 5회 = 60회, 콜드 캐시.
 *
 * 사전 등록: PREDICTION.md 부칙 3 (T1~T4)
 * 사용: node measure-eager.mjs [--repeat=5] [--site=all|paris|rome|london] [--resume=1]
 */
...
const SITES = {
  paris:  { url: 'https://en.wikivoyage.org/wiki/Paris',
            blocks: '.mw-parser-output section > *', intrinsic: 182 },
  rome:   { url: 'https://en.wikivoyage.org/wiki/Rome',
            blocks: '.mw-parser-output section > *', intrinsic: 182 },
  london: { url: 'https://en.wikivoyage.org/wiki/London',
            blocks: '.mw-parser-output section > *', intrinsic: 182 },
};

const MODES = ['plain', 'cv-auto'];
const CACHES = ['cold'];           // C8b 는 콜드만 — warm 에서는 lazy/eager 가 의미 없다
const LAZYS = ['asis', 'forced'];  // asis = 손 안 댐(히어로가 실제 eager) / forced = 히어로에도 lazy
...
const LOCK = path.join(import.meta.dirname, '.measure-eager.lock');
if (existsSync(LOCK)) {
...
const OUT = path.join(import.meta.dirname, 'results-eager.json');
const out = { experiment: 'c8c-real-eager', generatedAt: new Date().toISOString(),
...
console.log(`\nC8c eager 실제판 — 사이트 ${keys.length}곳 × 모드 ${MODES.length} × asis/forced × ${REPEAT}회 = ${order.length}회 (콜드)`);
console.log(`뷰포트 ${VIEWPORT.w}×${VIEWPORT.h} (모바일) · 기준선 chrome ${BASELINE}개 · 순서 섞음\n`);

let port = 46000 + Math.floor(Math.random() * 300);
let i = 0;
...
await save();
console.log('\n✅ results-eager.json 저장 —  node analyze-eager.mjs');
process.exit(0);
